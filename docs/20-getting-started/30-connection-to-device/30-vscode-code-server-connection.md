...
title: "Подключение через VSCode и code-server"
sidebar_label: "VSCode / code-server"
sidebar_position: 3
description: "Подключение к BRover-E5 через браузер (code-server) и VSCode Remote-SSH"
---

# Подключение через VSCode и code-server
...
* **VSCode + Remote-SSH** — полноценная локальная IDE с удалённым доступом

---

## Подключение через code-server (браузер)

На ровере запущен веб-сервис **code-server**, который предоставляет VSCode прямо в браузере.
...
После подключения откроется интерфейс VSCode, работающий напрямую на ровере.
...
## Подключение через VSCode (Remote-SSH)
...
9. После подключения начнётся установка VSCode Server на устройстве
